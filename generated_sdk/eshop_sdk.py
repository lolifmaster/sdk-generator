...
class APIClient:
    ...
    def _get_access_token(self) -> str:
        ...
        response: requests.Response = requests.post(
            url, data=json.dumps(self.credentials), headers=headers
        )
    ...
    def _make_request(
        ...
        if method == "get":
            ...
            response: requests.Response = requests.post(
                url, data=json.dumps(data), headers=full_headers
            )
        elif method == "delete":
    ...
    def order_partial_update(self, order_id: str, data: dict[str, Union[str, None]]):
        return self._make_authenticated_request(
            "patch", f"/api/orders/{order_id}/", data=data
        )

    def order_update(self, order_id: str, data: dict[str, Union[str, None]]):
        return self._make_authenticated_request(
            "put", f"/api/orders/{order_id}/", data=data
        )

    # Products Endpoints
    def products_list(self, cursor: str = None, search: str = None):
        params: dict[str, Union[str, None]] = (
            {"cursor": cursor, "search": search} if cursor or search else {}
        )
        return self._make_authenticated_request("get", "/api/products/", params=params)

    def products_read(self, product_id: str, search: str = None):
    ...
    # Token Endpoint
    def token_create(self, credentials: Credentials):
